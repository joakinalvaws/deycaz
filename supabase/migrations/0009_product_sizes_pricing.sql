...
-- y las migraciones 0002-0008. Ejecuta esto en el SQL Editor.
--
-- Antes, el precio de 3ml/10ml se calculaba solo (70%/170% del precio de
-- 5ml) y nunca se guardaba — no había forma de que un perfume se saliera
-- de esa fórmula. Ahora cada producto puede tener un precio propio para
-- 3ml/10ml (si no se completa, se sigue usando la fórmula como valor por
-- defecto) y, nuevo, un precio de "frasco entero" (perfume completo, no
-- decant) — opcional, si un producto no lo tiene configurado simplemente
-- no aparece esa opción en su página.
--
-- El frasco entero se compra individual (como cualquier decant hoy) pero
-- NO participa de "Arma tu Combo" — ahí se sigue eligiendo solo entre
-- 3/5/10ml, sin cambios.

alter table products
  add column if not exists price_3ml numeric,
  add column if not exists price_10ml numeric,
  add column if not exists price_full_bottle numeric;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'products_price_3ml_check') then
    alter table products add constraint products_price_3ml_check
      check (price_3ml is null or price_3ml > 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'products_price_10ml_check') then
    alter table products add constraint products_price_10ml_check
      check (price_10ml is null or price_10ml > 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'products_price_full_bottle_check') then
    alter table products add constraint products_price_full_bottle_check
      check (price_full_bottle is null or price_full_bottle > 0);
  end if;
end $$;

-- order_items.size pasa a admitir 'full' (era check inline, nombrado por
-- Postgres como <tabla>_<columna>_check).
alter table order_items drop constraint if exists order_items_size_check;
alter table order_items add constraint order_items_size_check
  check (size in ('3', '5', '10', 'full'));

-- ============================================================
-- place_order: mismo cuerpo que 0006, con precios de tamaño reales para
-- items individuales (con la fórmula vieja como fallback si no se
-- configuró un precio propio) y soporte de 'full' (nunca en combo).
-- ============================================================

...
-- y las migraciones 0002-0011. Ejecuta esto en el SQL Editor.
--
-- Topes de sanidad del pedido. `place_order` tiene `grant execute ... to
-- anon` (tiene que tenerlo: el checkout es público, sin login), así que
-- cualquiera puede llamarla directo por REST sin pasar por el sitio. Hasta
-- ahora `qty` solo tenía `check (qty > 0)` en order_items y `p_items` no
-- tenía tope de largo: se podían crear pedidos con qty gigante o con miles
-- de líneas, que después hay que limpiar a mano desde el panel.
--
-- Mantener sincronizado con MAX_QTY_PER_LINE / MAX_LINES_PER_ORDER en
-- src/app/actions.ts (ahí se cortan antes solo para dar un mensaje claro;
-- esta función es la validación autoritativa).
--
-- Único cambio respecto a la función de 0011: el bloque de validación de
-- `p_items` al inicio. La firma no cambia.
